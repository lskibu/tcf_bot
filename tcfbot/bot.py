...
class Bot:
    ...
    def login(self) -> None:
        ...
        while retires < MAX_RETRIES:
            try:
                ...
                req = self.session.post('https://portail.if-algerie.com/login',
                                        headers=head, data=data, verify=False)
        ...
        raise LoginError(json.dumps({
                'error': 'login error',
                'message': f'ConnectTimeoutError: Max retires exceeded : {retires} retries!'
        }))
    ...
    def reserve(self) -> Tuple[Event, PaymentDay]:
        ...
        for event in self.event_list.get_account_open_events(self.account):
            logger.info(f'Trying event: {event.uid}')
            for payment_day in self.payment_days.get_payment_days(event):
                logger.info(f'Trying payment day: {payment_day.time_shift_uid}')
                data = {
                        'uid': event.uid,
