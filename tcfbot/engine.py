...
class Engine:
    ...
    def next_account(self, bot: Bot):
        ...
        logger.info(f'taking reservation for account:{bot.account.email}')
        bot.run()
    ...
    def crawler_event_callback(self, sender, error, data):
        if error:
            ...
            logger.info(f'fetched {event_list.count()} events')
            ...
            logger.info(f'{open_event_list.count()} open events')
            # fetch payments now
            if self.abort:
                return self.trigger_engine_stopped()
            if open_event_list.count() > 0:
                logger.info('got open events, crawling payment days')
    ...
    def reserve_accounts(self, event_list: EventList, payment_day_list: PaymentDayList):
        while self.bot_count < self.max_bots and \
            ...
            if not account.reserved:
                logger.info(f'taking reservation for account:{account.email}')
